...
function show_hiscores ()
{
 ...
 get_hiscore_grep_string
 if [[ -n $(grep "$HISCORE_GREP_STRING" "$SHARE_DIR/hiscores.txt")    ]]
 then
  ...
  echo -e "╰───────────────────────────────────────╯"
 else echo -e "${C0}No statistics available just yet.${n}"
 fi
}
...
function new_word()
{
 ...
 case $DIFFICULTY in
  easy)WORD_DATABASE="$CONFIG_DIR/4l_word_database.psv";WORD_LENGTH=4;
  ;;
  medium)if [[ $ALTER_DB -eq 0 ]];then WORD_DATABASE="$CONFIG_DIR/4l_word_database.psv";WORD_LENGTH=4;ALTER_DB=1;else WORD_DATABASE="$CONFIG_DIR/5l_word_database.psv";WORD_LENGTH=5;ALTER_DB=0;fi;
  ;;
  hard)WORD_DATABASE="$CONFIG_DIR/5l_word_database.psv";WORD_LENGTH=5;
 esac
...
function end_game()
{
 ...
 end_round
 get_hiscore_grep_string
 if [[ $(grep -c "$HISCORE_GREP_STRING" "$SHARE_DIR/hiscores.txt") -lt 1 ]]
 then
 ...
 if [ "$SCORE" -gt "$TENTH" ]||[[ "$(grep -c "$HISCORE_GREP_STRING" "$SHARE_DIR/hiscores.txt")" -lt 10 ]]
 then
 any_key "continue"
...
function check_word()
{
 ...
 if [[ "$(echo -e $WORD0_SORTED|sort)" == "$(echo -e $WORD2_SORTED|sort)" ]]
 then
...
function play_menu()
{
 ...
 do
  ...
  print_box
  db2="$(/tmp/readsn1.sh)";
 if [[ $(echo "$db2" | od) == "$backspace" ]];then for ((i=$((WORD_LENGTH-1));i>-1;i--));do if [[ ${WORD2[i]} != "_" ]];then WORD2[i]="_";echo "WORD2=(${WORD2[@]})">/tmp/deshuffle_word2.txt;break;fi;done;fi
 if [[ $(echo "$db2" | od) == "$delete" ]];then for ((i=0;i<$WORD_LENGTH;i++));do WORD2[i]="_";done;echo "WORD2=(${WORD2[@]})">/tmp/deshuffle_word2.txt;fi
  case $db2 in
   "Q")kill "$(jobs -l|grep 'timer'|awk '{print $2}')">/dev/null 2>&1;clear;exit;
   ;;
   "M")kill "$(jobs -l|grep 'timer'|awk '{print $2}')">/dev/null 2>&1;clear;db2=Q;
   ;;
   [a-z]) clear;for ((i=0;i<$WORD_LENGTH;i++));do if [[ ${WORD2[i]} == "_" ]];then WORD2[i]="$db2";echo "WORD2=(${WORD2[@]})">/tmp/deshuffle_word2.txt;break;fi;done;
   ;;
   "I")clear;if [[ $LEGEND_TOGGLE == no ]];then LEGEND_TOGGLE=yes ;else LEGEND_TOGGLE=no;fi;echo "$LEGEND_TOGGLE">/tmp/deshuffle_LEGEND_TOGGLE.txt
   ;;
   "")kill "$(jobs -l|grep 'timer'|awk '{print $2}')">/dev/null 2>&1;check_word
   ;;
   *)clear;
  esac
 done
...
function main_menu ()
{
 ...
 do
  ...
  case $mm in
   n)clear;new_game; 
   ;;
   c)clear;eval $PREFERRED_EDITOR $CONFIG_FILE;echo -e "\0033[?25l";load_config
   ;;
   r)clear;show_rules;any_key "return to main menu"
   ;;
   h)clear;show_hiscores;any_key "return to main menu"
   ;;
   q)clear;
   ;;
   *)clear;
